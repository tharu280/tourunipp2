...
from clean_run.integrations.wiki_pageviews_client import get_attraction_monthly_interest
...
def _component_level_from_score(score: int) -> str:
    if score >= 24:
        return "high"
    if score >= 14:
        return "medium"
...
def _summarize_weather_pressure(route: dict[str, Any]) -> dict[str, Any]:
    ...
    for segment in segments:
        ...
        forecast_dates = forecast.get("dates") or []
        forecast_date = forecast_dates[day_index] if day_index < len(forecast_dates) else None
        flagged_days.append(
            {
                "date": forecast_date,
                "day": segment.get("day"),
...
def _summarize_tourism_demand(trip_dates: list[str]) -> dict[str, Any]:
    daily = [get_tourism_demand_for_date(iso_date) for iso_date in trip_dates]
    known = [item for item in daily if item.get("level") != "unknown"]
    if not known:
        return {
            "level": "unknown",
            "score": 0,
            "summary": "SLTDA tourism demand data is unavailable for the selected trip dates.",
            "daily": daily,
        }

    average_score = round(sum(int(item.get("score", 0) or 0) for item in known) / len(known))
    highest = max(known, key=lambda item: int(item.get("score", 0) or 0))

    return {
        "level": _component_level_from_score(average_score),
        "score": average_score,
        "summary": highest.get("summary") or "SLTDA tourism arrivals were used as a seasonal demand baseline.",
        "peak_date": highest.get("date"),
        "daily": daily,
    }
...
def _build_zone_pressure(
    *,
    ...
    tourism_demand_by_date: dict[str, dict[str, Any]],
) -> dict[str, Any]:
    segments = route.get("segments") or []
    day_rows = []
    ...
    for index, segment in enumerate(segments):
        ...
        tourism_demand = tourism_demand_by_date.get(iso_date, {})
        tourism_score = int(tourism_demand.get("score", 0) or 0)
        tourism_reasons = [tourism_demand["summary"]] if tourism_demand.get("summary") else []

        base_score = holiday_score + tourism_score + weather_score + road_score + traffic_score
        ...
        day_reasons = [*holiday_reasons, *tourism_reasons, *weather_reasons, *road_reasons, *traffic_reasons]
        if not day_reasons:
...
def _build_attraction_visit_windows(*, combined_score: int, weather_score: int) -> list[dict[str, Any]]:
    weather_afternoon_penalty = 5 if weather_score >= 30 else 0
    adjustments = [
        ("early_morning", -12, "lowest local crowd pressure for popular stops"),
        ("mid_morning", -5, "balanced timing before the heaviest day pressure"),
        ("afternoon", 8 + weather_afternoon_penalty, "usually busier and more exposed to heat/rain risk"),
        ("evening", 3, "moderate demand, but daylight and closing times can matter"),
    ]
    windows = []
    for label, adjustment, reason in adjustments:
        score = max(0, min(100, combined_score + adjustment))
        windows.append(
            {
                "label": label,
                "score": score,
                "quality": _window_level(score),
                "reason": reason,
            }
        )
...
def _combined_pressure_summary(*, level: str, score: int, components: dict[str, int]) -> str:
    active = [
        name.replace("_", " ")
        for name, value in components.items()
        if value > 0 and name not in {"base_day_pressure"}
    ]
    if active:
        return f"{_pressure_text(level)} attraction pressure ({score}/100), driven by {', '.join(active[:3])}."
    return f"{_pressure_text(level)} attraction pressure ({score}/100), mostly from the route-day demand baseline."
    ...
    *,
    ...
    for segment in route.get("segments") or []:
        ...
        for place in candidate_places:
            base_day_score = int(day_pressure.get("pressure_score", 0))
            pressure_score = base_day_score
            reasons = []

            weekend_score = 0
            if is_weekend:
                weekend_score = 4
                pressure_score += weekend_score
                reasons.append("weekend travel demand can make this attraction busier")
            holiday_score = 0
            if is_holiday:
                holiday_score = 8
                pressure_score += holiday_score
            ...
            wiki_interest = get_attraction_monthly_interest(place=place, iso_date=iso_date) if iso_date else {}
            wiki_score = int(wiki_interest.get("score", 0) or 0)
            if wiki_score:
                pressure_score += wiki_score
                reasons.append(wiki_interest.get("summary") or "Wikipedia pageviews suggest extra attraction interest")

            weather_exposure = _attraction_weather_exposure(place)
            weather_attraction_score = 0
            if weather_score >= 60:
                weather_attraction_score = weather_exposure
                pressure_score += weather_attraction_score
                reasons.append("forecast disruption can affect the comfort of this stop")
            elif weather_score >= 30:
                weather_attraction_score = max(weather_exposure // 2, 1)
                pressure_score += weather_attraction_score
            ...
            components = {
                "base_day_pressure": base_day_score,
                "weekend": weekend_score,
                "holiday": holiday_score,
                "attraction_popularity_proxy": iconic_bonus,
                "wikipedia_interest": wiki_score,
                "weather_exposure": weather_attraction_score,
                "route_distance_friction": distance_penalty,
            }
            visit_windows = _build_attraction_visit_windows(
            ...
            advisories.append(
                {
                    ...
                    "combined_pressure": {
                        ...
                        "summary": _combined_pressure_summary(
                            level=level,
                            score=pressure_score,
                            components=components,
                        ),
                    },
                    "preferred_visit_window": visit_windows[0]["label"] if visit_windows else day_pressure.get("preferred_visit_window", "mid_morning"),
                    "best_visit_window": visit_windows[0] if visit_windows else {},
                    "visit_window_scores": visit_windows,
...
def get_crowd_signals_for_route(
    *,
    ...
    tourism_demand_pressure = _summarize_tourism_demand(trip_dates)
    ...
    if tourism_demand_pressure["level"] != "unknown":
        reasons.append(tourism_demand_pressure["summary"])
    ...
    signal_score = (
        holiday_score
        + tourism_demand_pressure["score"]
        + weather_pressure["score"]
        + road_pressure["score"]
        + traffic_pressure["score"]
    )
    risk_level = _level_from_score(signal_score)
    if (
        fetch_warnings
        and not holiday_matches
        and not weekend_dates
        and tourism_demand_pressure["level"] == "unknown"
        and weather_pressure["level"] == "unknown"
        and road_pressure["level"] == "unknown"
        and traffic_pressure["level"] == "unknown"
    ):
        risk_level = "unknown"
    ...
    zone_pressure = _build_zone_pressure(
        ...
        tourism_demand_by_date={
            item["date"]: item
            for item in tourism_demand_pressure.get("daily", [])
            if item.get("date")
        },
    )
    ...
    if tourism_demand_pressure["level"] == "high":
        recommendations.append(
            "SLTDA arrival trends show a high-demand tourism week, so treat major attractions as busier than normal."
        )
    ...
    helper_summary = (
        ...
        f"Tourism demand is {tourism_demand_pressure['level']}, holiday demand is {holiday_level}, "
        f"weather disruption is {weather_pressure['level']}, "
    ...
    return {
        ...
        "components": {
            ...
            "tourism_demand_pressure": tourism_demand_pressure,
            "weather_pressure": weather_pressure,
            "road_pressure": road_pressure,
            "traffic_pressure": traffic_pressure,
        },
