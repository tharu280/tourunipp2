...
from google_places.client import search_nearby_places, search_text_places
from google_routes.polyline import haversine_meters, summarize_geometry
from google_routes.segments import build_day_segments, recommend_segment_query_plan
...
def load_curated_attractions() -> list[dict[str, Any]]:
    dataset_path = Path("data") / "sri_lanka_attractions.json"
...
def load_curated_accommodations() -> list[dict[str, Any]]:
    dataset_path = Path("data") / "sri_lanka_accommodations.json"
    payload = json.loads(dataset_path.read_text(encoding="utf-8"))
...
def normalize_curated_accommodation(
    *,
    ...
    rating_band_score = {
        ...
        "very_good": 4.3,
        "excellent": 4.7,
    }.get(str(accommodation.get("rating_band") or "").lower(), 3.5)

    return {
        "place_id": accommodation.get("id") or f"acc_{accommodation.get('name')}",
        "category": "lodging",
        "display_name": accommodation.get("name"),
        "formatted_address": f'{accommodation.get("nearby_area") or accommodation.get("district")}, {accommodation.get("province")}',
        "location": {
            "lat": accommodation.get("latitude"),
            "lng": accommodation.get("longitude"),
        },
        "google_maps_uri": None,
        "primary_type": accommodation.get("accommodation_type"),
        "types": [accommodation.get("accommodation_type")],
        "rating": rating_band_score,
        "rating_band": accommodation.get("rating_band"),
        "price_band": accommodation.get("price_band"),
        "tags": accommodation.get("tags", []),
        "ideal_for": accommodation.get("ideal_for", []),
        "corridor": accommodation.get("corridor"),
        "district": accommodation.get("district"),
        "province": accommodation.get("province"),
        "summary": accommodation.get("summary"),
...
def fetch_places_for_segment(
    *,
    ...
    try:
        if segment.get("is_overnight_stop") and strategy == "nearby":
        ...
        elif segment.get("is_overnight_stop"):
...
def enrich_route(route: dict[str, Any], *, trip_days: int, strategy: str) -> dict[str, Any]:
    geometry = summarize_geometry(route["polyline"])
    total_route_distance_m = float(route.get("distance_meters") or geometry["total_geometry_distance_m"])
    total_route_duration_seconds = parse_duration_seconds(route.get("duration"))
    route_pool_info = build_route_attraction_pool(
        geometry_points=geometry["decoded_points"],
        trip_days=trip_days,
    )
    ...
    for segment in segments:
        ...
        ranked = rank_places_for_segment(segment=segment, fetched_places=fetched_places)
...
def main() -> None:
    ...
    enriched_routes = [
        enrich_route(route, trip_days=args.trip_days, strategy=args.strategy)
